...
async function listEvidence(url, env) {
  ...
  if (stage) {
    ...
    where.push("EXISTS (SELECT 1 FROM evidence_stages s WHERE s.evidence_id=e.evidence_id AND s.stage=?)"); params.push(stage);
  }
  if (condition) {
    if (!CONDITIONS.has(condition)) return json({ error: "Invalid condition" }, 400);
    where.push("EXISTS (SELECT 1 FROM evidence_conditions c WHERE c.evidence_id=e.evidence_id AND c.condition=?)"); params.push(condition);
  }
  if (verdict) {
    if (!VERDICTS.has(verdict)) return json({ error: "Invalid verdict" }, 400);
    where.push("e.verdict=?"); params.push(verdict);
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/evidence" || url.pathname === "/api/evidence/") return listEvidence(url, env);
    if (url.pathname.startsWith("/api/evidence/")) {
      const id = decodeURIComponent(url.pathname.slice("/api/evidence/".length));
      if (!id || id.includes("/")) return json({ error: "Invalid evidence id" }, 400);
      return getEvidence(id, env);
    }
