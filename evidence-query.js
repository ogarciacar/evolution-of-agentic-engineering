(() => {
  ...
  function weakest(items) { return [...items].sort((a,b) => a.count-b.count || a.value.localeCompare(b.value))[0]; }
  function synthesis(records) {
    if (!records.length) return "";
    const stageCounts=countsFor(records,stages,(r)=>r.mapping?.stages||[]), conditionCounts=countsFor(records,conditions,(r)=>r.mapping?.conditions||[]), verdictCounts=countsFor(records,verdicts,(r)=>[r.model_implication?.verdict].filter(Boolean));
    const leadStage=strongest(stageCounts), leadCondition=strongest(conditionCounts), weakCondition=weakest(conditionCounts);
    const support=verdictCounts.find(({value})=>value==="SUPPORTS")?.count||0, refine=verdictCounts.find(({value})=>value==="REFINES")?.count||0, contradict=verdictCounts.find(({value})=>value==="CONTRADICTS")?.count||0, inconclusive=verdictCounts.find(({value})=>value==="INCONCLUSIVE")?.count||0;
    const verdictStatement=contradict?`${contradict} ${contradict===1?"signal challenges":"signals challenge"} the current model.`:refine?`${refine} ${refine===1?"signal refines":"signals refine"} the model without directly contradicting it.`:support?`${support} ${support===1?"signal supports":"signals support"} the model and none directly contradict it.`:`${inconclusive} ${inconclusive===1?"signal is":"signals are"} inconclusive.`;
